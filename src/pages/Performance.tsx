...
import { Plus, Search, Filter, TrendingUp, Edit, Send, Loader2 } from "lucide-react";
    ...
    <DashboardLayout>
      ...
      {filteredActive.length > 0 && (
        <div className="mb-8 rounded-xl border border-border bg-card overflow-hidden">
          ...
          <div className="px-6 py-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
            <span>Showing <strong>1 – {filteredActive.length}</strong> from <strong>{filteredActive.length}</strong></span>
            <div className="flex items-center gap-1">
      ...
      {filteredDrafts.length > 0 && (
        <div className="rounded-xl border border-border bg-card overflow-hidden">
            ...
            <tbody className="divide-y divide-border">
              {filteredDrafts.map(form => (
                <tr key={form.id} className="hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <Progress value={0} className="h-2 w-28" />
                  ...
                  <td className="px-6 py-4">
                    <Badge className={cn("border text-xs", statusStyles.draft)}>Draft</Badge>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-1">
          ...
          <div className="px-6 py-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
            <span>Showing <strong>1 – {filteredDrafts.length}</strong> Drafts from <strong>{filteredDrafts.length}</strong></span>
            <div className="flex items-center gap-1">
              <Button variant="outline" size="sm" className="h-7 w-7 p-0">‹</Button>
              <Button size="sm" className="h-7 w-7 p-0">1</Button>
              <Button variant="outline" size="sm" className="h-7 w-7 p-0">›</Button>
