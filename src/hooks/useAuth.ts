...
export function useAuth(): UseAuthReturn {
  ...
  useEffect(() => {
    ...
    authFetch("/api/auth/me")
      .then((res) => {
        if (!res.ok) { clearSession(); setToken(null); setUser(null); }
      })
  ...
  const login = useCallback(async (email: string, password: string) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.errors?.email?.[0] ?? data.message ?? "Login failed");
  ...
  const changePassword = useCallback(async (currentPassword: string, newPassword: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.errors?.current_password?.[0] ?? data.message ?? "Failed");
  ...
  return {
    ...
    login, register, logout, changePassword, clearError,
    isAdmin, isHR, isManager, isAccountant, canApproveLeave, canManagePayroll,
  };
}